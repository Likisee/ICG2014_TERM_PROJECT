...
class RippleFrame extends Frame implements ComponentListener, ActionListener, AdjustmentListener, MouseMotionListener, MouseListener, ItemListener {
	...
	class Doppler2Setup extends Setup {
		...
		void eachFrame() {
			...
			if (w2 == windowOffsetY + windowHeight / 4 || w2 == windowOffsetY + windowHeight * 3 / 4) {
				dir = -dir;
				waiting = 1000;
			}
		}
	...
	void setupMode(int x, int y, int sx, int sy, int nx, int ny) {
		...
		for (i = 0; i != sx; i++)
			for (j = 0; j != sy; j++) {
				int gi = i + x + gw * (j + y);
				func[gi] = (float) (Math.sin(pi * nx * (i + 1) / (sx + 1)) * Math.sin(pi * ny * (j + 1) / (sy + 1)));
				funci[gi] = 0;
			}
	}
	...
	void setupAcousticMode(int x, int y, int sx, int sy, int nx, int ny) {
		...
		for (i = 0; i != sx; i++)
			for (j = 0; j != sy; j++) {
				int gi = i + x + gw * (j + y);
				func[gi] = (float) (Math.cos(pi * nx * i / (sx - 1)) * Math.cos(pi * ny * j / (sy - 1)));
				funci[gi] = 0;
			}
	}
		...
		void select() {
			...
			while (y + ny < windowHeight) {
				...
				for (i = 0; i != nx; i++)
					for (j = 0; j != ny; j++) {
						int gi = i + x1 + gw * (j + y1);
						func[gi] = (float) (Math.sin(mode1 * pi * (i + 1) / (nx + 1)) * Math.sin(pi * (j + 1) / (ny + 1)) * .5 + Math.sin(mode2 * pi * (i + 1) / (nx + 1)) * Math.sin(pi * (j + 1) / (ny + 1)) * .5);
						funci[gi] = 0;
					}
				y += ny + 1;
			}
			...
			for (gx = 1; gx <= maxmode; gx++)
				for (gy = 1; gy <= maxmode; gy++) {
					...
					for (i = 0; i != nx; i++)
						for (j = 0; j != ny; j++) {
							int gi = i + x1 + gw * (j + y1);
							func[gi] = (float) (Math.sin(mode1x * pi * (i + 1) / (nx + 1)) * Math.sin(mode1y * pi * (j + 1) / (ny + 1)) * .5 + Math.sin(mode2x * pi * (i + 1) / (nx + 1)) * Math.sin(mode2y * pi * (j + 1) / (ny + 1)) * .5);
							funci[gi] = 0;
						}
				}
			dampingBar.setValue(1);
	...
	class Waveguides5Setup extends Waveguides3Setup {
		...
		void eachFrame() {
			...
			while (x + nx < windowWidth) {
				...
				for (j = 0; j != nx; j++)
					func[x1 + j + gw * y] *= .5 * (Math.sin(pi * n1 * (j + 1) / (nx + 1)) + Math.sin(pi * n2 * (j + 1) / (nx + 1)));
				x += nx + 1;
				g++;
			}
		}
