...
class WaveBoxFrame extends Frame implements ComponentListener, ActionListener, AdjustmentListener, MouseMotionListener, MouseListener, ItemListener {
	...
	public void init() {
		...
		add(brightnessBar = new Scrollbar(Scrollbar.HORIZONTAL, 240, 1, 1, 2000));
		brightnessBar.addAdjustmentListener(this);

		add(new Label("Image Resolution", Label.CENTER));
		add(resolutionBar = new Scrollbar(Scrollbar.HORIZONTAL, 100, 2, 20, 240));
	...
	void reinit() {
		...
		if (useBufferedImage) {
			try {
				...
				memimage = (Image) cstr.newInstance(new Object[] { new Integer(view3d.width), new Integer(view3d.height), new Integer(1) }); // BufferedImage.TYPE_INT_RGB)});
		...
		if (pixels == null) {
			...
			for (i = 0; i != view3d.width * view3d.height; i++)
				pixels[i] = 0xFF000000;
			imageSource = new MemoryImageSource(view3d.width, view3d.height, pixels, 0, view3d.width);
	...
	void computeFunction() {
		...
		for (i = 0; i != gridSizeX; i++)
			for (j = 0; j != gridSizeY; j++) {
				...
				double camnorm = java.lang.Math.sqrt(camvx0 * camvx0 + camvy0 * camvy0 + 1);
				int n;
				double simpr = 0;
				double simpg = 0;
	...
	void fillSquare(int i, int j, double cr, double cg, double cb) {
		...
		int colval = 0xFF000000 + (((int) cr) << 16) | (((int) cg) << 8) | (((int) cb));
		int y2l = y2 * view3d.width;
		for (k = x; k < x2; k++)
			for (l = y * view3d.width; l < y2l; l += view3d.width)
				pixels[k + l] = colval;
	...
	void drawCube(Graphics g, boolean drawAll) {
		...
		for (i = 0; i != 6; i++) {
			...
			if (slice != SLICE_NONE && i / 2 != slice - SLICE_X) {
				...
				g.drawLine(slicerPoints[0][sp], slicerPoints[1][sp], slicerPoints[0][sp + 1], slicerPoints[1][sp + 1]);
	...
	void unmap3d(double x3[], int x, int y, double pn[], double pp[]) {
		...
		double t = ((pp[0] - mx) * pn[0] + (pp[1] - my) * pn[1] + (pp[2] - mz) * pn[2]) / (pn[0] * mvx + pn[1] * mvy + pn[2] * mvz);
	...
	public void adjustmentValueChanged(AdjustmentEvent e) {
		...
		if (e.getSource() == freqBar || e.getSource() == aux1Bar || e.getSource() == aux2Bar || e.getSource() == aux3Bar)
			needsPrecompute = true;
		if (e.getSource() == resolutionBar && resolutionBar.getValue() != resBarValue) {
			setMaxTerms();
		...
		void precompute() {
			...
			for (x = 0; x != maxTerms; x++)
				for (y = 0; y != maxTerms; y++) {
					double xi = x - mxhalf;
					int yi = y - mxhalf;
					dataxy[x][y] = (int) (distmult * java.lang.Math.sqrt(xi * xi + yi * yi) + .5);
			...
			for (x = 0; x != maxTerms; x++)
				for (y = 0; y <= maxdist; y++) {
					int xi = x - mxhalf;
					double r = java.lang.Math.sqrt(y * y / (distmult * distmult) + xi * xi) * resadj + .00000001;
	...
	class PinholeSetup extends Setup {
		...
		void precompute() {
			...
			for (x = 0; x != maxTerms; x++)
				for (y = 0; y <= maxdist; y++) {
					double r = java.lang.Math.sqrt(y * y / (distmult * distmult) + x * x) * resadj + .00000001;
					datadzr[y][x] = java.lang.Math.cos(r * mult) / r;
					datadzi[y][x] = -java.lang.Math.sin(r * mult) / r;
				}
		}
	...
	class TwoSourcesSetup extends Setup {
		...
		void precompute() {
			...
			for (z = 0; z != maxTerms; z++)
				for (y = 0; y <= maxdist; y++) {
					int zi = z - mxhalf;
					double r = java.lang.Math.sqrt(y * y / (distmult * distmult) + zi * zi) * resadj + .0000001;
		...
		double computePoint(int x, int y, int z) {
			...
			return w1mult * (datadzr[d1][z] * cost1 - datadzi[d1][z] * sint1) + w2mult * (datadzr[d2][z] * cost2 - datadzi[d2][z] * sint2);
		}
	...
	class LateralQuadrupoleSetup extends Setup {
		...
		double computePoint(int x, int y, int z) {
			...
			return (datadzr[d1][z] + datadzr[d2][z] - datadzr[d3][z] - datadzr[d4][z]) * cost1 - (datadzi[d1][z] + datadzi[d2][z] - datadzi[d3][z] - datadzi[d4][z]) * sint1;
		}
	...
	class LinearQuadrupoleSetup extends Setup {
		...
		void precompute() {
			...
			for (x = 0; x != maxTerms; x++) {
				...
				for (y = 0; y != maxTerms; y++) {
					double yi = y - mxhalf + .001;
					dataxy1[x][y] = (int) (distmult * java.lang.Math.sqrt(xi1 * xi1 + yi * yi) + .5);
					dataxy2[x][y] = (int) (distmult * java.lang.Math.sqrt(xi2 * xi2 + yi * yi) + .5);
	...
	class TwoPinholesSetup extends Setup {
		...
		void precompute() {
			...
			for (x = 0; x != maxTerms; x++) {
				double xi = x - mxhalf + sep + .001;
				for (y = 0; y != maxTerms; y++) {
					dataxy[x][y] = (int) (distmult * java.lang.Math.sqrt(xi * xi + y * y) + .5);
					if (dataxy[x][y] > maxdist)
						maxdist = dataxy[x][y];
				}
			}
	...
	class DoubleLineSetup extends Setup {
		...
		double computePoint(int x, int y, int z) {
			int nx = mxlast - x;
			return w1mult * (datar[x][y] * cost1 - datai[x][y] * sint1) + w2mult * (datar[nx][y] * cost2 - datai[nx][y] * sint2);
		}
		...
		void precompute() {
			...
			for (x = 0; x != maxTerms; x++) {
				...
				for (y = 0; y != maxTerms; y++) {
					...
					datar[x][y] = m * (bessj0(r1 * mult) + bessj0(r2 * mult) + bessj0(r3 * mult));
					datai[x][y] = -m * (bessy0(r1 * mult) + bessy0(r2 * mult) + bessy0(r3 * mult));
				}
			}
		}
	...
	class PlaneWaveSetup extends Setup {
		...
		double computePoint(int x, int y, int z) {
			return .05 * (java.lang.Math.cos(x * mult) * cost1 + java.lang.Math.sin(x * mult) * sint1);
		}
	...
	class TwoPlaneWavesSetup extends Setup {
		...
		double computePoint(int x, int y, int z) {
			...
			return w1mult * (java.lang.Math.cos(k1) * cost1 + java.lang.Math.sin(k1) * sint1) + w2mult * (java.lang.Math.cos(k2) * cost2 + java.lang.Math.sin(k2) * sint2);
		}
	...
	double bessj0(double x) {
		...
		if (x < 8.0) {
			...
			ans = java.lang.Math.sqrt(0.636619772 / ax) * (java.lang.Math.cos(xx) * ans1 - z * java.lang.Math.sin(xx) * ans2);
		}
		return ans;
	}
