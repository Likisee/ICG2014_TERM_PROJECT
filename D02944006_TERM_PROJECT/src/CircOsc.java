...
class CircOscFrame extends Frame implements ComponentListener, ActionListener, AdjustmentListener, MouseMotionListener, MouseListener, ItemListener {
	...
	public void init() {
		...
		add(speedBar = new Scrollbar(Scrollbar.HORIZONTAL, 105, 1, 1, 250));
		...
		add(brightnessBar = new Scrollbar(Scrollbar.HORIZONTAL, 10, 1, 0, 100));
		brightnessBar.addAdjustmentListener(this);

		add(new Label("Resolution", Label.CENTER));
		add(resBar = new Scrollbar(Scrollbar.HORIZONTAL, 16, 1, 2, maxSampleCount / 2));
		...
		add(phasorBar = new Scrollbar(Scrollbar.HORIZONTAL, 10, 1, 5, 66));
	...
	void setupDisplay() {
		...
		case DISP_3D:
			if (!freqCheck.getState())
				...
				viewFreq = new View(0, winSize.height / 2, winSize.width, winSize.height / 2);
			}
			break;
		case DISP_2D:
			if (!freqCheck.getState())
		...
		default:
			if (!freqCheck.getState()) {
				view3d = new View(0, 0, winSize.width, winSize.height / 2);
				view2d = new View(0, winSize.height / 2, winSize.width, winSize.height / 2);
			} else {
				view3d = new View(0, 0, winSize.width / 2, winSize.height / 2);
				view2d = new View(winSize.width / 2, 0, winSize.width / 2, winSize.height / 2);
				viewFreq = new View(0, winSize.height / 2, winSize.width, winSize.height / 2);
			}
			break;
		}
	...
	void setupRaster(View v) {
		...
		if (java2) {
			try {
				...
				v.memimage = (Image) cstr.newInstance(new Object[] { new Integer(v.width), new Integer(v.height), new Integer(1) }); // BufferedImage.TYPE_INT_RGB)});
		...
		if (v.pixels == null) {
			...
			v.imageSource = new MemoryImageSource(v.width, v.height, v.pixels, 0, v.width);
	...
	void doPluck(double val) {
		...
		int i, j, x, y;
		double b = java.lang.Math.sqrt(selectedGridX * selectedGridX + selectedGridY * selectedGridY);
		if (b >= 1)
			return;
		...
		double fudge = .0001;
		double mulout = val / (java.lang.Math.log(fudge) - java.lang.Math.log(imageb + fudge) - subout);
		for (x = 0; x != sampleCountR; x++)
			for (y = 0; y != sampleCountTh; y++) {
				...
				double rfunc = (java.lang.Math.log(r1 + fudge) - java.lang.Math.log(r2 + fudge) - subout) * mulout;
				func[y][x] = rfunc;
			}
		transform();
		cv.repaint(pause);
	...
	void transform() {
		...
		for (r = 0; r <= sampleCountR; r++) {
			...
			for (j = 0; j != modeCountR; j++) {
				...
				phasecoef[0][j] += modeFuncsR[j][r] * modeFuncsR[j][r] * r * sampleCountTh;
			}
			...
			for (i = 1; i < modeCountTh; i += 2) {
				for (j = 0; j != modeCountR; j++) {
					...
					magcoef[i][j] += modeFuncsTh[i2][j][r] * .5 * (xformbuf[ii] + xformbuf[wm & (-ii)]);
					// sin component contributes to even i modes
					magcoef[i + 1][j] += modeFuncsTh[i2][j][r] * .5 * (xformbuf[ii + 1] - xformbuf[wm & (-ii + 1)]);
					// calculate norm. also multiply in 1/2 which is the
					// result of integrating sin or cos around the circle.
					phasecoef[i][j] += modeFuncsTh[i2][j][r] * modeFuncsTh[i2][j][r] * r * sampleCountTh * .5;
	...
	public void updateCircOsc(Graphics realg) {
		...
		if (view3d != null) {
			...
			if (display2Chooser.getSelectedIndex() == DISP2_SOLID) {
				...
				for (i = 0; i != sc2; i++) {
					...
					double normdot = (qx + qy + 1) * (1 / 1.73) / java.lang.Math.sqrt(qx * qx + qy * qy + 1);
					int col = computeColor(x, y, normdot);
					fillTriangle(view3d, xpoints[0], ypoints[0], xpoints[1], ypoints[1], xpoints[3], ypoints[3], col);
					fillTriangle(view3d, xpoints[0], ypoints[0], xpoints[2], ypoints[2], xpoints[3], ypoints[3], col);
				}
		...
		if (view2d != null) {
			...
			for (y = 0; y != sampleCountR; y++) {
				...
				for (x = 0; x != sampleCountTh; x++) {
					...
					fillTriangle(view2d, xpoints[0], ypoints[0], xpoints[1], ypoints[1], xpoints[2], ypoints[2], col);
					fillTriangle(view2d, xpoints[0], ypoints[0], xpoints[2], ypoints[2], xpoints[3], ypoints[3], col);
					xpoints[0] = xpoints[1];
					ypoints[0] = ypoints[1];
					xpoints[3] = xpoints[2];
					ypoints[3] = ypoints[2];
		...
		/*
		 * long t3 = System.currentTimeMillis(); System.out.println("q " +
		 * (t3-t2));
		 */
		if (viewFreq != null) {
			...
			for (i = 0; i <= maxDispRModes; i++) {
				x = i * termWidth;
				g.drawLine(viewFreq.x, x + viewFreq.y, viewFreq.x + termWidth * maxDispThModes, x + viewFreq.y);
			}
			for (i = 0; i <= maxDispThModes; i++) {
				x = i * termWidth;
				g.drawLine(viewFreq.x + x, viewFreq.y, viewFreq.x + x, viewFreq.y + termWidth * maxDispRModes);
			}
			int rcol = 0x00010000;
			int gcol = 0x00000100;
			for (i = 0; i != maxDispThModes; i++)
				for (j = 0; j != maxDispRModes; j++) {
					...
					if (selectedCoefX != -1 && omega[selectedCoefX][selectedCoefY] == omega[i][j]) {
		...
		for (r = 0; r <= sampleCountR; r++) {
			...
			for (i = 1; i < modeCountTh; i += 2) {
				...
				for (j = 0; j != modeCountR; j++) {
					...
					ds += modeFuncsTh[i2][j][r] * magcoef[i + 1][j] * phasecoefcos[i + 1][j];
				}
				// put in cosine modes
				xformbuf[ii * 2] = .5 * dc;
	...
	void fillTriangle(View view, int x1, int y1, int x2, int y2, int x3, int y3, int col) {
		if (x1 > x2) {
			if (x2 > x3) {
				// x1 > x2 > x3
	...
	void editMag(int x, int y) {
		...
		if (pcoef > 2 * pi)
			pcoef = 2 * pi;
		if (magcoef[selectedCoefX][selectedCoefY] == coef && phasecoefadj[selectedCoefX][selectedCoefY] == pcoef)
			return;
	...
	void setResolution() {
		...
		System.out.print("grid: " + sampleCountTh + " " + sampleCountR + " " + sampleCountTh * sampleCountR + " " + modeCountTh + " " + modeCountR + "\n");
		// System.out.println(sth + " " + modeCountTh);
		scaleHeight = 6;
	...
	double zeroj(int m_order, int n_zero) {
		...
		double z = beta - (mu - 1) / beta8 - 4 * (mu - 1) * (7 * mu - 31) / (3 * beta82 * beta8);
	...
	void setDamping() {
		...
		for (i = 0; i != modeCountTh; i++) {
			for (j = 0; j != modeCountR; j++) {
				...
				double damp2 = omega[i][j] * java.lang.Math.sqrt(java.lang.Math.sqrt(1 + damper * damper / (omega[i][j] * omega[i][j])) - 1);
	...
	void findGridPoint2D(int mx, int my) {
		...
		double r = java.lang.Math.sqrt(selectedGridX * selectedGridX + selectedGridY * selectedGridY);
		if (r > 1) {
			selectedGridX /= r;
			selectedGridY /= r;
		}
	...
	void findGridPoint3D(int mx, int my) {
		...
		for (y = 0; y <= sampleCountR; y++)
			for (x = 0; x <= sampleCountTh; x++) {
				...
				if (r < bestr) {
					bestr = r;
					double th = (x + sampleCountTh / 2) * 2 * pi / sampleCountTh - viewAngle;
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (e.getItemSelectable() == displayChooser || e.getItemSelectable() == freqCheck) {
			setupDisplay();
			cv.repaint(pause);
		}
		if (e.getItemSelectable() == display2Chooser || e.getItemSelectable() == colorCheck)
			cv.repaint(pause);
	...
	void doPlay() {
		...
		try {
			...
			Constructor cstr = afclass.getConstructor(new Class[] { float.class, int.class, int.class, boolean.class, boolean.class });
			Object format = cstr.newInstance(new Object[] { new Float(rate), new Integer(8), new Integer(1), new Boolean(true), new Boolean(true) });
			...
			Method opmeth = sdlclass.getMethod("open", new Class[] { afclass, int.class });
			opmeth.invoke(line, new Object[] { format, new Integer(playSampleCount) });
			Method stmeth = sdlclass.getMethod("start", null);
			stmeth.invoke(line, null);
			Method wrmeth = sdlclass.getMethod("write", new Class[] { b.getClass(), int.class, int.class });
